...
class TestProcessEmptyResponse(unittest.TestCase):
    ...
    @parameterized.expand([["success_code", 200, True, {}], ["error_code", 401, False, None]])
...
class TestUseRefreshToken(unittest.TestCase):
    """Class to test the use refresh token method."""
...
class TestGetListId(unittest.TestCase):
    """Class to test the get list id method."""
...
class TestEpochToUTC(unittest.TestCase):
    """Class to test the epoch to UTC method."""
...
class TestAddListValueToCEF(unittest.TestCase):
    """Class to test the add list value to CEF method."""
...
class TestExtractCyberValues(unittest.TestCase):
    """Class to test the extract cyber values method."""
...
class TestCreateAlertArtifacts(unittest.TestCase):
    """Class to test the create alert artifacts method."""
...
class TestProcessAlertData(unittest.TestCase):
    """Class to test the process alert data method."""
    ...
    def test_process_alert_data_valid_method(self, name, alert, expected_value):
        ...
        if "valid_container_valid_artifacts" == name:
            self.connector.save_container.return_value = (True, "", 1)
            self.connector.save_artifacts.return_value = (True, "", 1)
            response = self.util._process_alert_data(self.action_result, alert)
            self.assertEqual(response, expected_value)

        if "valid_container_invalid_artifacts" == name:
            self.connector.save_container.return_value = (True, "", 1)
            self.connector.save_artifacts.return_value = (False, "", 1)
            response = self.util._process_alert_data(self.action_result, alert)
            self.assertEqual(response, expected_value)

        if "invalid_container_valid_artifacts" == name:
            self.connector.save_container.return_value = (False, "", 1)
            self.connector.save_artifacts.return_value = (True, "", 1)
            response = self.util._process_alert_data(self.action_result, alert)
            self.assertEqual(response, expected_value)

        if "invalid_container_invalid_artifacts" == name:
            self.connector.save_container = Mock(return_value=(False, "", 1))
            self.connector.save_artifacts = Mock(return_value=(False, "", 1))
            response = self.util._process_alert_data(self.action_result, alert)
            self.assertEqual(response, expected_value)
